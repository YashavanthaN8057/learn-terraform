...
resource "null_resource" "main" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-pull -U https://github.com/YashavanthaN8057/learn-ansible.git -e env=${var.env} -e component=${var.name} expense.yaml"
    ]
  }
}
...
resource "aws_route53_record" "main" {
  zone_id = var.zone_id
  name    = "${var.name}-${var.env}.cloudcera.shop"
  type    = "A"
  ttl     = 10
